...
module.exports = (chapter) => {
  ...
  const pages = [
    ...
    L.page('Level 1 · Level 2', `
      ${youDo('Set B. Stems and leaves.', `
        <div class="short-grid" style="grid-template-rows: repeat(3, auto)">
          ${qs(1, 'Stem and leaf of 47?')}${qs(2, 'Stem and leaf of 63?')}${qs(3, 'Stem and leaf of 105?')}
          ${qs(4, 'Stem and leaf of 9?')}${qs(5, 'Order the leaves: 5 | 8 2 6 1')}${qs(6, 'Order the leaves: 7 | 4 0 9 3 3')}
        </div>
        ${split(qDraw(7, 'Draw an ordered stem-and-leaf plot of:<br>23, 31, 27, 35, 22, 40, 38, 31, 26, 44', G.stemLeafTemplate({ stems: [2, 3, 4] })),
          `<div class="short-list" style="grid-template-rows: repeat(3, auto); align-content: start; gap: 3mm">${qs(8, 'How many values are in the 30s?')}${qs(9, 'What is the mode?')}${qs(10, 'What is the highest value?')}</div>`)}`)}
      ${banner(2)}
      ${weDo(split(
        example('Example 3', 'Rewrite this unordered plot as an ordered plot.', `<div class="split">${G.stemLeaf({ rows: [[6, [8, 2, 5]], [7, [3, 0, 9, 1]], [8, [4, 4, 0]]] })}${G.stemLeafTemplate({ stems: [6, 7, 8], key: false })}</div>`),
        example('Example 4', 'Draw an ordered stem-and-leaf plot of: 56, 43, 61, 48, 55, 59, 62, 47, 55', G.stemLeafTemplate({ stems: 3 }))))}
      ${youDo('Set C. Masses of 20 rugby players (kg). Questions 2–9 are on the next page.', split(graphCard(G.stemLeaf({ title: 'Unordered plot', rows: rugby, key: '8 | 4 = 84 kg' })),
        qDraw(1, 'How many players are there? Rewrite the plot as an ordered stem-and-leaf plot.', G.stemLeafTemplate({ stems: [7, 8, 9, 10] })), 'wide-right'), 'grow')}
    `),

    L.page('Level 2 · Level 3', `
      ${youDo('Set C continued. Use the rugby plot on the previous page.', `<div class="work-grid r4">${setC.map((t, i) => q(i + 2, t)).join('')}</div>`, 'grow')}
      ${banner(3)}
      ${weDo(split(
    ...
    L.page('Level 3', `
      ${youDo('Set D. Decimals and 3-digit numbers.', `
        <p class="given"><b>100 m sprint times (seconds):</b> ${times.map((t) => t.toFixed(1)).join(', ')}</p>
        ${split(`<div class="col">${qDraw(1, 'Draw an ordered stem-and-leaf plot of the times. Include a key.', G.stemLeafTemplate({ stems: 5 }))}${q(2, 'How many runners were there?')}</div>`,
          `<div class="col">${q(3, 'What was the fastest time?')}${q(4, 'What is the mode?')}${q(5, 'How many ran under 14 seconds?')}${q(6, 'What fraction ran 15 seconds or slower?')}</div>`, 'grow')}
        <p class="given"><b>Scores in a video game:</b> ${scores.join(', ')}</p>
        ${split(`<div class="col">${qDraw(7, 'Draw an ordered stem-and-leaf plot of the scores. Include a key.', G.stemLeafTemplate({ stems: 6 }))}${q(8, 'Which stems did you use? Why?')}</div>`,
          `<div class="col">${q(9, 'Why must a stem-and-leaf plot have a key?')}${q(10, 'Give one advantage of a stem-and-leaf plot over a column graph.')}</div>`, 'grow')}`, 'grow')}
    `),

    L.page('Extension · Exit ticket', `
      ${banner(4)}
      ${split(worked(4, 'Worked example: split stems',
        'Many values have the same stem: 41, 42, 42, 44, 45, 46, 47, 47, 48, 49, 52, 53. Draw a split-stem plot.',
        ['Split each stem into two rows', 'Row “4” holds leaves 0 to 4; row “4*” holds leaves 5 to 9', 'Do the same for 5 if you need to'],
        '<b>Answer:</b> the plot is easier to read because the long row is split',
        ...
        youDo('read the worked example, then try these.', `<div class="stack">
          ...
          ${q('E3', 'When is it useful to split the stems?')}</div>`, 'fill'), 'ext grow')}
    ...
    L.page('Summary', `
      ...
      <div class="summary-grid">
        <div class="steps-card"><b>Drawing a stem-and-leaf plot</b><ol><li>Choose the <b>stems</b> (usually the tens) and write them down the left.</li><li>Write each <b>leaf</b> (the units) next to its stem.</li><li>Rewrite with the leaves <b>in order</b>, smallest first.</li><li>Always add a <b>key</b>, e.g. 3 | 5 = 35.</li></ol></div>
      </div>
  ...
  const answers = [
    ...
    ['Set C', [`${rg.length} players; ordered: ${ordered(rugby)}`, `${Math.min(...rg)} kg`, `${Math.max(...rg)} kg`, `${S.modeText(rg)} kg`, `${count(rg, (x) => x < 80)}`, `${count(rg, (x) => x >= 90)}`,
      `${count(rg, (x) => x >= 80 && x < 90)}/20`, `${count(rg, (x) => x >= 100)}/20 = 10%`, 'You can see the lowest, highest and most common values straight away']],
    ['Set D', ['13 | 5 8 8, 14 | 2 2 2 6 9, 15 | 1 4, 16 | 0 (key 13 | 5 = 13.5 s)', `${times.length}`, `${Math.min(...times)} s`, `${S.modeText(times)} s`, `${count(times, (x) => x < 14)}`, `${count(times, (x) => x >= 15)}/11`,
